---
title: "Estimating Posterior Model Probabilities via Bayesian Model Based Sampling"
...
- prior distributions on all unknowns $(\b_{\Mg}, \Mg, \alpha_{\Mg}, \phi_{\Mg})$ and turn the Bayesian crank to get posterior distributions!





- for **nice** priors, we can integrate out the parameters $\tg = (\bg, \alpha_{\Mg}, \phi_{\Mg})$ to obtain the marginal likelihood of $\Mg$ 
\begin{align*}
...
$$
- estimation $\E[\mub \mid \Y]$,  $p(\Y^* \mid \Y)$, marginal inclusion probabilities $P(\gamma_j = 1 \mid \Y)$
...
- inclusion probability that $\g_i \in S$ - under sampling with replacement
...
- Hájek (1971) estimator uses an auxilary variable $A_i > 0$, where we expect $p(\Y \mid \M_i)p(\M_i) \propto A_i$,  with $A \equiv \sum_{i = 1}^{N} A_i$
$$\hat{C} = \frac{\sum_{i=1}^n \frac{p(\Y \mid \M_i)p(\M_i)} {\pi_i}} {\sum_{i=1}^n\frac{A_i/A}{\pi_i}}$$ may be preferable when $p(\Y \mid \M_i)p(\M_i)$ are  weakly correlated with $\pi_i$ or  when $n$ is not fixed
...
- using the posterior predictive for $m_i \notin S$, $\E[m_i \mid m_j \in S] = \pi_i \hat{\beta}$
\begin{align*}
...
- Basu (1971) approach works with PPS-WOR take $\pi_i \propto A_i \equiv q(\g_i)$ (adaptation?)
